...
std::string MCPPromptDefinition::GeneratePrompt(const json& args) const {
    ...
    if (name == "analyze-crash") {
        ...
        oss << "Perform a systematic crash analysis at: " << crash_address << "\n\n"
            ...
            << "6. Use `memory_read` on the stack area around the current stack pointer (`csp`, ~256 bytes) for stack contents.\n"
            ...
            << "- Stack overflow (RSP on x64 / ESP on x86 outside the stack region)\n"
        ...
        oss << "Set up comprehensive tracing for function: " << function_name << "\n\n"
            ...
            << "   x64: \"EXIT " << function_name << " | RAX={RAX}\"\n"
            << "   x86: \"EXIT " << function_name << " | EAX={EAX}\"\n"
        ...
        oss << "Compare debugger state at two different execution points.\n\n"
            ...
            << "4. Use `disassembly_at` at the current instruction pointer (`cip`, count=10) to record position.\n"
            ...
            << "8. Use `disassembly_at` at the new instruction pointer (`cip`, count=10).\n"
        ...
        oss << "Initialize a debugging session. Issue: " << issue << "\n\n"
            ...
            << "6. If paused at a valid address, use `disassembly_at` with the current instruction pointer (`cip`, count=20).\n"
